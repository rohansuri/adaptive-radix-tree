...
class Node16 extends Node4 {
	final static int NODE_SIZE = 16;
	// private byte[] keys = new byte[NODE_SIZE];
	...
	Node16(Node4 node) {
		...
		System.arraycopy(child, 1, this.child, 1, node.noOfChildren);
	...
	Node16(Node48 node48) {
		...
		for (int i = 0, j = 0; i < Node48.KEY_INDEX_SIZE; i++) {
			if (keyIndex[i] != Node48.ABSENT) {
				child[j+1] = children[keyIndex[i]+1];
				keys[j] = BinaryComparableUtils.unsigned(child[j+1].uplinkKey());
				replaceUplink(this, child[j+1]);
				j++;
			}
		}
	}
